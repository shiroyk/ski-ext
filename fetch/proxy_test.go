...
func TestSOCKS5Proxy(t *testing.T) {
	...
	for _, useTLS := range []bool{false, true} {
		t.Run(fmt.Sprintf("useTLS=%v", useTLS), func(t *testing.T) {
			...
			go proxy(t)
			c := ts.Client()
			c.Transport.(*http.Transport).Proxy = ProxyFromRequest
			ctx := WithRoundRobinProxy(context.Background(), "socks5://"+l.Addr().String())
			request, err := http.NewRequestWithContext(ctx, http.MethodHead, ts.URL, nil)
			if err != nil {
				t.Fatal(err)
			}
...
func TestTransportProxy(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		t.Run(fmt.Sprintf("httpsSite=%v, httpsProxy=%v", httpsSite, httpsProxy), func(t *testing.T) {
			...
			c.Transport.(*http.Transport).Proxy = ProxyFromRequest
			ctx := WithRoundRobinProxy(context.Background(), proxy1.URL, proxy2.URL)
			request, err := http.NewRequestWithContext(ctx, http.MethodHead, ts.URL, nil)
			if err != nil {
				t.Fatal(err)
			}
